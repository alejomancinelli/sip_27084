# Estructura canónica de la telemetría del template

**Este documento NO describe lo que publica este equipo.** Describe la estructura que trae
el template genérico y que usan los **proyectos nuevos**. Lo que publica este equipo está en
[`influxdb.md`](influxdb.md).

Está acá porque esta instalación divergió: el dashboard de la cinta ya existía cuando el
proyecto se migró al template, y renombrar las series habría dejado la historia con el
nombre viejo y los paneles sin datos. Se conservaron los nombres viejos y se guardó esta
referencia, para que el próximo fork arranque de la estructura buena y no copie la
divergencia de éste.

Las diferencias, en una tabla:

| | Template (este documento) | Este equipo ([`influxdb.md`](influxdb.md)) |
|---|---|---|
...
| Tag de cámara | `camera` | `camara_id` |
...
| Red | `net_<iface>_rx_mbps` | `rx_<etiqueta>` |
| Salud de la óptica | campos de `camera` | measurement `optica` |
| Medias de la hora | no existen | `<métrica>_1h` + `cobertura_pct` |

Lo que sigue es la estructura del template, sin cambios.
